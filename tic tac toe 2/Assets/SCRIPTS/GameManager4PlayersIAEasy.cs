...
public class GameManager4PlayersIAEasy : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (currentTurn == PlayerType.AI2)
        {
            AI2_EasyTurn();
        }

        if (currentTurn == PlayerType.AI3)
        {
            AI3_EasyTurn();
        }
    }
    ...
    void AI2_EasyTurn()
    {
        ...
        if (availableSpots.Count > 0)
        {
            int randomIndex = Random.Range(0, availableSpots.Count);
            GameObject randomSpot = availableSpots[randomIndex];
            randomSpot.GetComponent<Renderer>().material.color = Color.yellow; // Assuming blue is the AI's colour
            ChangeTurn();
        }
    }
    ...
    void AI3_EasyTurn()
    {
        ...
        foreach (GameObject obj in clickableObjects)
        {
            if (obj.GetComponent<Renderer>().material.color != Color.red && obj.GetComponent<Renderer>().material.color != Color.blue && obj.GetComponent<Renderer>().material.color != Color.yellow)
            {
                availableSpots.Add(obj);
            }
        }
        ...
        if (availableSpots.Count > 0)
        {
            int randomIndex = Random.Range(0, availableSpots.Count);
            GameObject randomSpot = availableSpots[randomIndex];
            randomSpot.GetComponent<Renderer>().material.color = Color.green; // Assuming blue is the AI's colour
            ChangeTurn();
        }
    }
